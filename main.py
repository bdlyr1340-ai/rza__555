...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    log.info('Received /start from user_id=%s username=%s', update.effective_user.id if update.effective_user else None, update.effective_user.username if update.effective_user else None)
    await send_home(update, context)


async def ping_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    await ensure_user(update, context)
    await update.effective_message.reply_text('pong ✅ البوت متصل')
...
async def run() -> None:
    ...
    # هذا السطر يحل أشهر سبب لعدم استجابة البوت: بقاء Webhook قديم على نفس التوكن.
    ...
    log.info('Telegram polling started successfully. Send /start or /ping to @%s', me.username)
